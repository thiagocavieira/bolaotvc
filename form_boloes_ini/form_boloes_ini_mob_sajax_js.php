  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_boloes_ini_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_boloes_ini_mob_form();
	}
...
?>
  ...
  // ---------- Validate nome_bolao
  function do_ajax_form_boloes_ini_mob_validate_nome_bolao()
  {
    var nomeCampo_nome_bolao = "nome_bolao";
    var var_nome_bolao = scAjaxGetFieldText(nomeCampo_nome_bolao);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_boloes_ini_mob_validate_nome_bolao(var_nome_bolao, var_script_case_init, do_ajax_form_boloes_ini_mob_validate_nome_bolao_cb);
  } // do_ajax_form_boloes_ini_mob_validate_nome_bolao

  function do_ajax_form_boloes_ini_mob_validate_nome_bolao_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_boloes_ini_mob_validate_nome_bolao_cb

  // ---------- Validate adm_bolao_id
  function do_ajax_form_boloes_ini_mob_validate_adm_bolao_id()
  {
    var nomeCampo_adm_bolao_id = "adm_bolao_id";
    var var_adm_bolao_id = scAjaxGetFieldText(nomeCampo_adm_bolao_id);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_boloes_ini_mob_validate_adm_bolao_id(var_adm_bolao_id, var_script_case_init, do_ajax_form_boloes_ini_mob_validate_adm_bolao_id_cb);
  } // do_ajax_form_boloes_ini_mob_validate_adm_bolao_id

  function do_ajax_form_boloes_ini_mob_validate_adm_bolao_id_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "adm_bolao_id";
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_boloes_ini_mob_validate_adm_bolao_id_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_form_boloes_ini_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_form_boloes_ini_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_nome_bolao = scAjaxGetFieldText("nome_bolao");
    var var_adm_bolao_id = scAjaxGetFieldText("adm_bolao_id");
    var var_bolao_id = scAjaxGetFieldText("bolao_id");
    ...
    scAjaxProcOn();
    x_ajax_form_boloes_ini_mob_submit_form(var_nome_bolao, var_adm_bolao_id, var_bolao_id, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_boloes_ini_mob_submit_form_cb);
  } // do_ajax_form_boloes_ini_mob_submit_form

  function do_ajax_form_boloes_ini_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    ...
    if (scAjaxIsOk())
    {
      ...
      scAjaxHideErrorDisplay("adm_bolao_id");
      scLigEditLookupCall();
<?php
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_form_boloes_ini_mob_submit_form_cb_after_alert);
  } // do_ajax_form_boloes_ini_mob_submit_form_cb
  function do_ajax_form_boloes_ini_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_boloes_ini_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_boloes_ini_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_form_boloes_ini_mob_navigate_form, function() {});
    } else {
      perform_ajax_form_boloes_ini_mob_navigate_form();
    }
  }

  function perform_ajax_form_boloes_ini_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    scAjaxHideErrorDisplay("nome_bolao");
    scAjaxHideErrorDisplay("adm_bolao_id");
    var var_bolao_id = document.F2.bolao_id.value;
    ...
    scAjaxProcOn();
    x_ajax_form_boloes_ini_mob_navigate_form(var_bolao_id, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_boloes_ini_mob_navigate_form_cb);
  } // do_ajax_form_boloes_ini_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_boloes_ini_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_boloes_ini_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.bolao_id.value = scAjaxGetKeyValue("bolao_id");
    scAjaxSetSummary();
    scAjaxShowDebug();
    scAjaxSetLabel(true);
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_boloes_ini_mob_navigate_form_cb_after_alert);
  } // do_ajax_form_boloes_ini_mob_navigate_form_cb
  function do_ajax_form_boloes_ini_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
...
{
?>
    do_ajax_form_boloes_ini_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_form_boloes_ini_mob_navigate_form_cb_after_alert

  function sc_hide_form_boloes_ini_mob_form()
  {
  ...
  ajax_field_list[1] = "adm_bolao_id";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "nome_bolao": {"label": "Nome do Bolão", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5},
    "adm_bolao_id": {"label": "Administrador do Bolão", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  var ajax_field_mult = {
    ...
    "adm_bolao_id": new Array()
  };
  ajax_field_mult["nome_bolao"][1] = "nome_bolao";
  ajax_field_mult["adm_bolao_id"][1] = "adm_bolao_id";

  var ajax_field_id = {
    "nome_bolao": new Array("hidden_field_label_nome_bolao", "hidden_field_data_nome_bolao"),
    "adm_bolao_id": new Array("hidden_field_label_adm_bolao_id", "hidden_field_data_adm_bolao_id")
  };

...
class cons_apostas_geral_xls
{
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          if (!is_file($this->Ini->root . $this->Ini->path_link . "cons_apostas_geral/cons_apostas_geral_res_xls.class.php"))
          {
              $this->Tem_xls_res  = false;
          }
          if ($this->Tem_xls_res)
          { 
              require_once($this->Ini->path_aplicacao . "cons_apostas_geral_res_xls.class.php");
              $this->Res_xls = new cons_apostas_geral_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_cons_apostas_geral.zip";
          $this->Arquivo   .= "_cons_apostas_geral" . $this->Xls_tp;
          $this->Tit_doc    = "cons_apostas_geral" . $this->Xls_tp;
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_return']);
          if ($this->Tem_xls_res) {
   ...
   function grava_arquivo()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['field_order'] as $Cada_cmp)
      {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_name']))
      {
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_name'], ".");
          if ($Pos === false) {
              $_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_name'] .= $this->Xls_tp;
          }
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
          $this->Zip_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arq_zip;
      }
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
...
$_SESSION['scriptcase']['cons_apostas_geral']['contr_erro'] = 'off'; 
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['embutida'])
         { 
      ...
      $this->xls_set_style();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   { 
      ...
      { 
          ...
          if ($Cada_col == "apostas_aposta_id" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $this->count_span++;
          ...
          if ($Cada_col == "apostas_dt_hr_aposta" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $this->count_span++;
              $current_cell_ref = $this->calc_cell($this->Xls_col);
              $SC_Label = NM_charset_to_utf8($SC_Label);
   ...
   function NM_export_apostas_aposta_id()
   {
         ...
         $this->apostas_aposta_id = NM_charset_to_utf8($this->apostas_aposta_id);
         if (is_numeric($this->apostas_aposta_id))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->apostas_aposta_id);
         $this->Xls_col++;
   }
   ...
   function NM_export_apostas_dt_hr_aposta()
   {
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
      if (!empty($this->apostas_dt_hr_aposta))
      {
         ...
         $this->apostas_dt_hr_aposta = NM_charset_to_utf8($this->apostas_dt_hr_aposta);
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->apostas_dt_hr_aposta, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->apostas_dt_hr_aposta, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- jogos_rodada
   ...
   //----- apostas_aposta_id
   function NM_sub_cons_apostas_aposta_id()
   {
         $this->apostas_aposta_id = NM_charset_to_utf8($this->apostas_aposta_id);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->apostas_aposta_id;
   ...
   //----- apostas_dt_hr_aposta
   function NM_sub_cons_apostas_dt_hr_aposta()
   {
      if (!empty($this->apostas_dt_hr_aposta))
      {
         if (substr($this->apostas_dt_hr_aposta, 10, 1) == "-") 
         ...
         $this->apostas_dt_hr_aposta = NM_charset_to_utf8($this->apostas_dt_hr_aposta);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->apostas_dt_hr_aposta;
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['nolabel'])
       {
 ...
 function quebra_apostas_login_id_sc_free_group_by($Cmp_qb, $Where_qb, $Cmp_Name) 
 {
   ...
   function quebra_jogos_competicao_id_sc_free_group_by_bot()
   {
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['field_order'] as $Cada_cmp)
       {
           if (!isset($this->NM_cmp_hidden[$Cada_cmp]) || $this->NM_cmp_hidden[$Cada_cmp] != "off")
           {
               ...
               elseif ($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['embutida']) {
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = "";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
   ...
   function quebra_apostas_login_id_sc_free_group_by_top()
   {
       if ($this->groupby_show != "S") {
           return;
       }
       ...
       $temp_cmp = "";
       $cont_col = 0;
       foreach ($this->campos_quebra_apostas_login_id as $cada_campo) {
           if ($cont_col == $lim_col) {
               $temp_cmp = html_entity_decode($temp_cmp, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
               $temp_cmp = strip_tags($temp_cmp);
               $temp_cmp = NM_charset_to_utf8($temp_cmp);
   ...
   function quebra_apostas_login_id_sc_free_group_by_bot()
   {
       if ($this->groupby_show != "S") {
           return;
       }
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_file'] = $this->Xls_f;
      }
...
<INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['cons_apostas_geral']['xls_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php

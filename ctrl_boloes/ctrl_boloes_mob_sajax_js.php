  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_ctrl_boloes_mob" == sTestField)
        {
...
?>
  ...
  // ---------- Validate sel_bolao
  function do_ajax_ctrl_boloes_mob_validate_sel_bolao()
  {
    var nomeCampo_sel_bolao = "sel_bolao";
    var var_sel_bolao = scAjaxGetFieldSelect(nomeCampo_sel_bolao);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_ctrl_boloes_mob_validate_sel_bolao(var_sel_bolao, var_script_case_init, do_ajax_ctrl_boloes_mob_validate_sel_bolao_cb);
  } // do_ajax_ctrl_boloes_mob_validate_sel_bolao

  function do_ajax_ctrl_boloes_mob_validate_sel_bolao_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "sel_bolao";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_ctrl_boloes_mob_validate_sel_bolao_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_ctrl_boloes_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_ctrl_boloes_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_sel_bolao = scAjaxGetFieldSelect("sel_bolao");
    ...
    scAjaxProcOn();
    x_ajax_ctrl_boloes_mob_submit_form(var_sel_bolao, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_ctrl_boloes_mob_submit_form_cb);
  } // do_ajax_ctrl_boloes_mob_submit_form

  function do_ajax_ctrl_boloes_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_ctrl_boloes_mob_submit_form_cb_after_alert);
  } // do_ajax_ctrl_boloes_mob_submit_form_cb
  function do_ajax_ctrl_boloes_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_ctrl_boloes_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_ctrl_boloes_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_ctrl_boloes_mob_navigate_form, function() {});
    } else {
      perform_ajax_ctrl_boloes_mob_navigate_form();
    }
  }

  function perform_ajax_ctrl_boloes_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_ctrl_boloes_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['ctrl_boloes_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_ctrl_boloes_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_ctrl_boloes_mob_navigate_form_cb_after_alert);
  } // do_ajax_ctrl_boloes_mob_navigate_form_cb
  function do_ajax_ctrl_boloes_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_ctrl_boloes_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_ctrl_boloes_mob_navigate_form_cb_after_alert

  function sc_hide_ctrl_boloes_mob_form()
  {
  ...
  ajax_field_list[0] = "sel_bolao";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "sel_bolao": {"label": "Selecione o bolão que deseja apostar", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  ajax_field_mult["sel_bolao"][1] = "sel_bolao";

  var ajax_field_id = {
    "sel_bolao": new Array("hidden_field_label_sel_bolao", "hidden_field_data_sel_bolao")
  };

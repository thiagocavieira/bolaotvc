  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_temporadas_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_temporadas_mob_form();
	}
...
?>
  ...
  function do_ajax_form_temporadas_mob_validate_temporada_id()
  {
    var nomeCampo_temporada_id = "temporada_id";
    var var_temporada_id = scAjaxGetFieldHidden(nomeCampo_temporada_id);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_temporadas_mob_validate_temporada_id(var_temporada_id, var_script_case_init, do_ajax_form_temporadas_mob_validate_temporada_id_cb);
  } // do_ajax_form_temporadas_mob_validate_temporada_id

  function do_ajax_form_temporadas_mob_validate_temporada_id_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_temporadas_mob_validate_temporada_id_cb

  // ---------- Validate ano_temp
  function do_ajax_form_temporadas_mob_validate_ano_temp()
  {
    var nomeCampo_ano_temp = "ano_temp";
    var var_ano_temp = scAjaxGetFieldText(nomeCampo_ano_temp);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_temporadas_mob_validate_ano_temp(var_ano_temp, var_script_case_init, do_ajax_form_temporadas_mob_validate_ano_temp_cb);
  } // do_ajax_form_temporadas_mob_validate_ano_temp

  function do_ajax_form_temporadas_mob_validate_ano_temp_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "ano_temp";
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_temporadas_mob_validate_ano_temp_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_form_temporadas_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_form_temporadas_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_temporada_id = scAjaxGetFieldHidden("temporada_id");
    var var_ano_temp = scAjaxGetFieldText("ano_temp");
    ...
    scAjaxProcOn();
    x_ajax_form_temporadas_mob_submit_form(var_temporada_id, var_ano_temp, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_temporadas_mob_submit_form_cb);
  } // do_ajax_form_temporadas_mob_submit_form

  function do_ajax_form_temporadas_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    ...
    if (scAjaxIsOk())
    {
      ...
      scAjaxHideErrorDisplay("ano_temp");
      scLigEditLookupCall();
<?php
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_form_temporadas_mob_submit_form_cb_after_alert);
  } // do_ajax_form_temporadas_mob_submit_form_cb
  function do_ajax_form_temporadas_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_temporadas_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_temporadas_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_form_temporadas_mob_navigate_form, function() {});
    } else {
      perform_ajax_form_temporadas_mob_navigate_form();
    }
  }

  function perform_ajax_form_temporadas_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    scAjaxHideErrorDisplay("temporada_id");
    scAjaxHideErrorDisplay("ano_temp");
    var var_temporada_id = document.F2.temporada_id.value;
    ...
    scAjaxProcOn();
    x_ajax_form_temporadas_mob_navigate_form(var_temporada_id, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_temporadas_mob_navigate_form_cb);
  } // do_ajax_form_temporadas_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_temporadas_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_temporadas_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.temporada_id.value = scAjaxGetKeyValue("temporada_id");
    scAjaxSetSummary();
    scAjaxShowDebug();
    scAjaxSetLabel(true);
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_temporadas_mob_navigate_form_cb_after_alert);
  } // do_ajax_form_temporadas_mob_navigate_form_cb
  function do_ajax_form_temporadas_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
...
{
?>
    do_ajax_form_temporadas_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_form_temporadas_mob_navigate_form_cb_after_alert

  function sc_hide_form_temporadas_mob_form()
  {
  ...
  ajax_field_list[0] = "temporada_id";
  ajax_field_list[1] = "ano_temp";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "temporada_id": {"label": "Temporada Id", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5},
    "ano_temp": {"label": "Ano Temp", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  ajax_field_mult["ano_temp"][1] = "ano_temp";

  var ajax_field_id = {
    "temporada_id": new Array("hidden_field_label_temporada_id", "hidden_field_data_temporada_id"),
    "ano_temp": new Array("hidden_field_label_ano_temp", "hidden_field_data_ano_temp")
  };

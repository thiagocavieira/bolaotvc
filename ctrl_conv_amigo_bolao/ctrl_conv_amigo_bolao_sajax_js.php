  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_ctrl_conv_amigo_bolao" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_ctrl_conv_amigo_bolao_form();
	}
...
?>
  ...
  function do_ajax_ctrl_conv_amigo_bolao_validate_nome_convidado()
  {
    var nomeCampo_nome_convidado = "nome_convidado";
    var var_nome_convidado = scAjaxGetFieldText(nomeCampo_nome_convidado);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_ctrl_conv_amigo_bolao_validate_nome_convidado(var_nome_convidado, var_script_case_init, do_ajax_ctrl_conv_amigo_bolao_validate_nome_convidado_cb);
  } // do_ajax_ctrl_conv_amigo_bolao_validate_nome_convidado

  function do_ajax_ctrl_conv_amigo_bolao_validate_nome_convidado_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_ctrl_conv_amigo_bolao_validate_nome_convidado_cb

  // ---------- Validate email_user
  function do_ajax_ctrl_conv_amigo_bolao_validate_email_user()
  {
    var nomeCampo_email_user = "email_user";
    var var_email_user = scAjaxGetFieldText(nomeCampo_email_user);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_ctrl_conv_amigo_bolao_validate_email_user(var_email_user, var_script_case_init, do_ajax_ctrl_conv_amigo_bolao_validate_email_user_cb);
  } // do_ajax_ctrl_conv_amigo_bolao_validate_email_user

  function do_ajax_ctrl_conv_amigo_bolao_validate_email_user_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_ctrl_conv_amigo_bolao_validate_email_user_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_ctrl_conv_amigo_bolao_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_ctrl_conv_amigo_bolao_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_ctrl_conv_amigo_bolao_submit_form(var_nome_convidado, var_email_user, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_ctrl_conv_amigo_bolao_submit_form_cb);
  } // do_ajax_ctrl_conv_amigo_bolao_submit_form

  function do_ajax_ctrl_conv_amigo_bolao_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_ctrl_conv_amigo_bolao_submit_form_cb_after_alert);
  } // do_ajax_ctrl_conv_amigo_bolao_submit_form_cb
  function do_ajax_ctrl_conv_amigo_bolao_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_ctrl_conv_amigo_bolao_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_ctrl_conv_amigo_bolao_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_ctrl_conv_amigo_bolao_navigate_form, function() {});
    } else {
      perform_ajax_ctrl_conv_amigo_bolao_navigate_form();
    }
  }

  function perform_ajax_ctrl_conv_amigo_bolao_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_ctrl_conv_amigo_bolao_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['ctrl_conv_amigo_bolao']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_ctrl_conv_amigo_bolao_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_ctrl_conv_amigo_bolao_navigate_form_cb_after_alert);
  } // do_ajax_ctrl_conv_amigo_bolao_navigate_form_cb
  function do_ajax_ctrl_conv_amigo_bolao_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_ctrl_conv_amigo_bolao_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_ctrl_conv_amigo_bolao_navigate_form_cb_after_alert

  function sc_hide_ctrl_conv_amigo_bolao_form()
  {

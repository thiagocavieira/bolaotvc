  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_ctrl_gravar_classificacao" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_ctrl_gravar_classificacao_form();
	}
...
?>
  ...
  // ---------- Validate competicao
  function do_ajax_ctrl_gravar_classificacao_validate_competicao()
  {
    var nomeCampo_competicao = "competicao";
    var var_competicao = scAjaxGetFieldSelect(nomeCampo_competicao);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_ctrl_gravar_classificacao_validate_competicao(var_competicao, var_script_case_init, do_ajax_ctrl_gravar_classificacao_validate_competicao_cb);
  } // do_ajax_ctrl_gravar_classificacao_validate_competicao

  function do_ajax_ctrl_gravar_classificacao_validate_competicao_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_ctrl_gravar_classificacao_validate_competicao_cb

  // ---------- Validate sel_oper
  function do_ajax_ctrl_gravar_classificacao_validate_sel_oper()
  {
    var nomeCampo_sel_oper = "sel_oper";
    var var_sel_oper = scAjaxGetFieldSelect(nomeCampo_sel_oper);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_ctrl_gravar_classificacao_validate_sel_oper(var_sel_oper, var_script_case_init, do_ajax_ctrl_gravar_classificacao_validate_sel_oper_cb);
  } // do_ajax_ctrl_gravar_classificacao_validate_sel_oper

  function do_ajax_ctrl_gravar_classificacao_validate_sel_oper_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_ctrl_gravar_classificacao_validate_sel_oper_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_ctrl_gravar_classificacao_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_ctrl_gravar_classificacao_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_ctrl_gravar_classificacao_submit_form(var_competicao, var_sel_oper, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_ctrl_gravar_classificacao_submit_form_cb);
  } // do_ajax_ctrl_gravar_classificacao_submit_form

  function do_ajax_ctrl_gravar_classificacao_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_ctrl_gravar_classificacao_submit_form_cb_after_alert);
  } // do_ajax_ctrl_gravar_classificacao_submit_form_cb
  function do_ajax_ctrl_gravar_classificacao_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_ctrl_gravar_classificacao_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_ctrl_gravar_classificacao_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_ctrl_gravar_classificacao_navigate_form, function() {});
    } else {
      perform_ajax_ctrl_gravar_classificacao_navigate_form();
    }
  }

  function perform_ajax_ctrl_gravar_classificacao_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_ctrl_gravar_classificacao_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['ctrl_gravar_classificacao']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_ctrl_gravar_classificacao_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_ctrl_gravar_classificacao_navigate_form_cb_after_alert);
  } // do_ajax_ctrl_gravar_classificacao_navigate_form_cb
  function do_ajax_ctrl_gravar_classificacao_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_ctrl_gravar_classificacao_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_ctrl_gravar_classificacao_navigate_form_cb_after_alert

  function sc_hide_ctrl_gravar_classificacao_form()
  {

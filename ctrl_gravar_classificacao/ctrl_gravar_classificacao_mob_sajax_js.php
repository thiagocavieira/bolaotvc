  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_ctrl_gravar_classificacao_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_ctrl_gravar_classificacao_mob_form();
	}
...
?>
  ...
  // ---------- Validate competicao
  function do_ajax_ctrl_gravar_classificacao_mob_validate_competicao()
  {
    var nomeCampo_competicao = "competicao";
    var var_competicao = scAjaxGetFieldSelect(nomeCampo_competicao);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_ctrl_gravar_classificacao_mob_validate_competicao(var_competicao, var_script_case_init, do_ajax_ctrl_gravar_classificacao_mob_validate_competicao_cb);
  } // do_ajax_ctrl_gravar_classificacao_mob_validate_competicao

  function do_ajax_ctrl_gravar_classificacao_mob_validate_competicao_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "competicao";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_ctrl_gravar_classificacao_mob_validate_competicao_cb

  // ---------- Validate sel_oper
  function do_ajax_ctrl_gravar_classificacao_mob_validate_sel_oper()
  {
    var nomeCampo_sel_oper = "sel_oper";
    var var_sel_oper = scAjaxGetFieldSelect(nomeCampo_sel_oper);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_ctrl_gravar_classificacao_mob_validate_sel_oper(var_sel_oper, var_script_case_init, do_ajax_ctrl_gravar_classificacao_mob_validate_sel_oper_cb);
  } // do_ajax_ctrl_gravar_classificacao_mob_validate_sel_oper

  function do_ajax_ctrl_gravar_classificacao_mob_validate_sel_oper_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "sel_oper";
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sAppErrors = scAjaxListErrors(true);
    if ("" == sAppErrors)
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_ctrl_gravar_classificacao_mob_validate_sel_oper_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_ctrl_gravar_classificacao_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_ctrl_gravar_classificacao_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_competicao = scAjaxGetFieldSelect("competicao");
    var var_sel_oper = scAjaxGetFieldSelect("sel_oper");
    ...
    scAjaxProcOn();
    x_ajax_ctrl_gravar_classificacao_mob_submit_form(var_competicao, var_sel_oper, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_ctrl_gravar_classificacao_mob_submit_form_cb);
  } // do_ajax_ctrl_gravar_classificacao_mob_submit_form

  function do_ajax_ctrl_gravar_classificacao_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_ctrl_gravar_classificacao_mob_submit_form_cb_after_alert);
  } // do_ajax_ctrl_gravar_classificacao_mob_submit_form_cb
  function do_ajax_ctrl_gravar_classificacao_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_ctrl_gravar_classificacao_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_ctrl_gravar_classificacao_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_ctrl_gravar_classificacao_mob_navigate_form, function() {});
    } else {
      perform_ajax_ctrl_gravar_classificacao_mob_navigate_form();
    }
  }

  function perform_ajax_ctrl_gravar_classificacao_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_ctrl_gravar_classificacao_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['ctrl_gravar_classificacao_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_ctrl_gravar_classificacao_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_ctrl_gravar_classificacao_mob_navigate_form_cb_after_alert);
  } // do_ajax_ctrl_gravar_classificacao_mob_navigate_form_cb
  function do_ajax_ctrl_gravar_classificacao_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_ctrl_gravar_classificacao_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_ctrl_gravar_classificacao_mob_navigate_form_cb_after_alert

  function sc_hide_ctrl_gravar_classificacao_mob_form()
  {
  ...
  ajax_field_list[0] = "competicao";
  ajax_field_list[1] = "sel_oper";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "competicao": {"label": "Selecione a competição:", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5},
    "sel_oper": {"label": "Operação", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  var ajax_field_mult = {
    "competicao": new Array(),
    "sel_oper": new Array()
  };
  ajax_field_mult["competicao"][1] = "competicao";
  ajax_field_mult["sel_oper"][1] = "sel_oper";

  var ajax_field_id = {
    "competicao": new Array("hidden_field_label_competicao", "hidden_field_data_competicao"),
    "sel_oper": new Array("hidden_field_label_sel_oper", "hidden_field_data_sel_oper")
  };
